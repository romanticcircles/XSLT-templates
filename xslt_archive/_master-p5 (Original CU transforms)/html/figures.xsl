...
<xsl:stylesheet exclude-result-prefixes="exsl estr edate svg a fo local rng tei teix xd m" extension-element-prefixes="exsl estr edate" version="2.0"
	...
	xmlns:exsl="http://exslt.org/common" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:svg="http://www.w3.org/2000/svg"
	xmlns:html="http://www.w3.org/1999/xhtml" xmlns:local="http://www.pantor.com/ns/local" xmlns:m="http://www.w3.org/1998/Math/MathML"
	xmlns:rng="http://relaxng.org/ns/structure/1.0" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:teix="http://www.tei-c.org/ns/Examples"
	xmlns:xd="http://www.pnp-software.com/XSLTdoc" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xd:doc type="stylesheet">
		<xd:short> TEI stylesheet dealing with elements from the figures module, making HTML output. </xd:short>
				...
				<div xmlns="http://www.w3.org/1999/xhtml" align="center">
					...
					<xsl:if test="tei:head">
						<div xmlns="http://www.w3.org/1999/xhtml" class="caption">
							...
							<p xmlns="http://www.w3.org/1999/xhtml" class="caption">
					...
					<xsl:choose>
						<xsl:when test="@rend='noThumb'">
							<xsl:apply-templates select="tei:graphic"/>
							<p xmlns="http://www.w3.org/1999/xhtml" class="caption">
								<xsl:if test="@n">
									<strong><xsl:text>Figure </xsl:text><xsl:value-of select="@n"/>:</strong><xsl:text> </xsl:text>
								</xsl:if>
								<xsl:apply-templates select="tei:figDesc"/>
							</p>
						</xsl:when>
						
						<xsl:when test="descendant-or-self::tei:graphic">
							<a xmlns="http://www.w3.org/1999/xhtml" class="colorbox">
								<xsl:attribute name="href">
									<xsl:value-of select="concat(substring-before(tei:graphic/@url, 'Thumb'), substring-after(tei:graphic/@url, 'Thumb'))"/>
								</xsl:attribute>
								<xsl:apply-templates select="tei:graphic"/>
							</a>
							<p xmlns="http://www.w3.org/1999/xhtml" class="caption">
					    ...
					    <xsl:when test="descendant-or-self::tei:media">
	...
	<xsl:template match="tei:figure/tei:head" mode="ok"/>
	...
	<xsl:template match="tei:hi">
		<xsl:choose>
			...
			<xsl:when test="@rend='hr'"> </xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates/>
			</xsl:otherwise>
		</xsl:choose>
	...
	<xsl:template name="showGraphic">
		...
		<xsl:choose>
			<xsl:when test="$showFigures='true'">
				<img xmlns="http://www.w3.org/1999/xhtml" src="{$graphicsPrefix}{$File}">
...
-->
  ...
  <xsl:template match="tei:media">
    <xsl:choose>
      <xsl:when test="@type='embed' and @mimeType=contains(@mimeType, 'audio')">
      ...
      <xsl:when test="@type='embed' and @mimeType=contains(@mimeType, 'video')">
