...
<xsl:stylesheet exclude-result-prefixes="exsl estr edate a fo local rng tei teix xd html" extension-element-prefixes="exsl estr edate" version="1.0"
	...
	<xd:doc type="stylesheet">
		<xd:short> TEI stylesheet dealing with elements from the linking module, making HTML output. </xd:short>
	...
	<xsl:template match="*" mode="ident">
		...
		<xsl:choose>
			...
			<xsl:otherwise><xsl:value-of select="$BaseFile"/>-<xsl:value-of select="local-name(.)"/>-<xsl:value-of select="generate-id()"
			/></xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<xd:doc>
	...
	<xd:doc>
		...
		<xd:detail>
			<p> when a &lt;div&gt; is referenced, see whether its plain anchor, or needs a parent HTML name prepended </p>
		</xd:detail>
	</xd:doc>
	...
	<xsl:template match="tei:label|tei:figure|tei:table|tei:item|tei:p|tei:bibl|tei:anchor|tei:cell|tei:lg|tei:list|tei:sp|tei:head" mode="generateLink">
		<xsl:variable name="ident">
			<xsl:apply-templates mode="ident" select="."/>
		</xsl:variable>
...
-->
	...
	<xsl:template name="locateParent">
		<xsl:choose>
			<xsl:when test="self::tei:div">
				<xsl:apply-templates mode="ident" select="ancestor::tei:div[last() - $splitLevel + 1]"/>
			</xsl:when>
			<xsl:when test="ancestor::tei:div">
				<xsl:apply-templates mode="ident" select="ancestor::tei:div[last() - $splitLevel]"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when test="$splitLevel = 0">
						<xsl:apply-templates mode="ident" select="ancestor::tei:div1|ancestor::tei:div0"/>
					</xsl:when>
					<xsl:when test="$splitLevel = 1">
						<xsl:apply-templates mode="ident" select="ancestor::tei:div2|ancestor::tei:div1|ancestor::tei:div0"/>
					</xsl:when>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:choose>
					...
					<xsl:when test="$splitLevel = 1">
						<xsl:apply-templates mode="ident" select="(ancestor::tei:div2|ancestor::tei:div1|ancestor::tei:div0)[last()]"/>
					</xsl:when>
					<xsl:when test="$splitLevel = 2">
	...
	<xsl:template name="makeExternalLink">
		...
		<a xmlns="http://www.w3.org/1999/xhtml">
			...
			<!-- Some special handling for RC when we want the link to open in a lightbox [DR]-->
			<xsl:if test="@rend='colorbox-node'"><xsl:attribute name="data-inner-width">
				<xsl:text>500</xsl:text>
			</xsl:attribute>
				<xsl:attribute name="data-inner-height">
					<xsl:text>500</xsl:text>
